 ...
 *   HexaCloud.accessStatus()     -> Promise({trial_ends_at,trial_active,credits})
...
(function () {
  ...
  // Crée un dossier via la RPC (atomique). Pendant l'essai : GRATUIT ; après :
  // consomme 1 crédit. Le back-end lève 'ACCES_REQUIS' quand l'essai est terminé
  // ET le solde nul (on tolère aussi l'ancien nom 'CREDIT_INSUFFISANT').
      ...
      return c.rpc("create_etude", { p_client: doc.client || "", p_titre: doc.title || "Étude patrimoniale", p_data: data || {} })
        .then(function (r) {
          if (r.error) {
            ...
            if (/ACCES_REQUIS|CREDIT_INSUFFISANT/.test(m)) throw new Error("ACCES_REQUIS");
            throw new Error(m);
          }
  ...
  // État d'accès (essai + crédits) de l'utilisateur courant, pour le tableau de
  // bord : { trial_ends_at, trial_active, credits }.
  function accessStatus() {
    return client().then(function (c) {
      return c.rpc("access_status").then(function (r) { if (r.error) throw r.error; return r.data || null; });
    });
  }
